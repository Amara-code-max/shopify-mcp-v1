...
class MetafieldInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    namespace: str = Field(..., description="Metafield namespace, e.g. 'custom', 'global'")
    key:       str = Field(..., description="Metafield key, e.g. 'accords_json'")
    value:     str = Field(..., description="Metafield value (always a string; JSON-stringify if needed)")
    type:      str = Field(..., description="Shopify metafield type, e.g. 'single_line_text_field'")


class SetMetafieldsInput(BaseModel):
    model_config = ConfigDict(extra="forbid")
    product_id:  int                  = Field(..., description="Shopify product ID")
    metafields:  List[MetafieldInput] = Field(..., description="List of metafields to create or update")
...
async def shopify_set_metafields(params: SetMetafieldsInput) -> str:
    """Create or update metafields on a product. POST is safe for both create and update (Shopify upserts on namespace+key)."""
    try:
        results = []
        for mf in params.metafields:
            body = {"metafield": {"namespace": mf.namespace, "key": mf.key, "value": mf.value, "type": mf.type}}
            try:
                data   = await _request("POST", f"products/{params.product_id}/metafields.json", body=body)
                saved  = data.get("metafield", {})
                results.append({
                    "namespace": saved.get("namespace"),
                    "key":       saved.get("key"),
                    "id":        saved.get("id"),
                    "status":    "created" if saved.get("created_at") == saved.get("updated_at") else "updated",
                })
            except Exception as inner:
                results.append({"namespace": mf.namespace, "key": mf.key, "status": "error", "message": str(inner)})
        return _fmt(results)
    except Exception as e:
        return _error(e)
...
async def shopify_get_metafields(params: GetMetafieldsInput) -> str:
    """Get all metafields for a product, with an optional namespace filter."""
    try:
        query_params: Dict[str, Any] = {}
        if params.namespace:
            query_params["namespace"] = params.namespace
        data = await _request("GET", f"products/{params.product_id}/metafields.json", params=query_params)
        return _fmt(data.get("metafields", data))
    except Exception as e:
        return _error(e)
...
class ListFilesInput(BaseModel):
    model_config = ConfigDict(extra="forbid")
    limit:    Optional[int] = Field(default=250, ge=1, le=250, description="Max files to return (1-250)")
    since_id: Optional[int] = Field(default=None, description="Pagination: return files after this ID")
    fields:   Optional[str] = Field(default="id,src,filename,created_at", description="Comma-separated fields to include")
...
async def shopify_list_files(params: ListFilesInput) -> str:
    """List files from the Shopify store via the REST Admin API."""
    try:
        p: Dict[str, Any] = {"limit": params.limit}
...
class ListFilesGraphQLInput(BaseModel):
    model_config = ConfigDict(extra="forbid")
    first: Optional[int] = Field(default=250, ge=1, le=250, description="Files per page (1-250)")
    after: Optional[str] = Field(default=None, description="Pagination cursor from a previous pageInfo.endCursor")
    query: Optional[str] = Field(default=None, description="Filter string e.g. 'media_type:IMAGE'")


@mcp.tool(
    name="shopify_list_files_graphql",
    annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": True},
)
async def shopify_list_files_graphql(params: ListFilesGraphQLInput) -> str:
    """List files via the Shopify GraphQL Admin API with cursor-based pagination. Returns edges and pageInfo."""
    try:
        ...
        body = {"query": _LIST_FILES_QUERY, "variables": variables}
